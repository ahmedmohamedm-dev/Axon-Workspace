...
class AppState {
    ...
    init {
        ...
        if (cached != null && cached.projects.isNotEmpty()) {
            workspace = cached
            val validId = if (cached.projects.any { it.id == cached.activeId }) {
                cached.activeId
            } else {
                cached.projects.firstOrNull()?.id
            }
        ...
        Timer(80) { restoreLinkedWorkspace() }
            .apply {
                isRepeats = false
                start()
            }
    ...
    fun currentProject(): Project? =
        workspace.projects.firstOrNull { it.id == selectedProjectId }

    fun currentTree(): Node? =
        currentProject()?.tree
    ...
    fun mutate(block: () -> Unit) {
        ...
        if (p != null && selectedNodeIds.isNotEmpty()) {
            val valid = selectedNodeIds
                .filter { TreeOps.findNode(p.tree, it) != null }
                .toSet()

            if (valid != selectedNodeIds) {
    ...
    private fun snapshotJson(): String =
        Persistence.json.encodeToString(
    ...
    fun undo() {
        ...
        workspace = Persistence.json.decodeFromString(
            Workspace.serializer(),
            snap.json
        )
        ...
        selectedProjectId =
            if (workspace.projects.any { it.id == workspace.activeId }) {
                workspace.activeId
            } else {
    ...
    private fun restoreSelection(
        ...
        val p = workspace.projects.firstOrNull {
        ...
        val anchor =
            if (
                prevId != null &&
                TreeOps.findNode(p.tree, prevId) != null
            ) {
                prevId
            } else {
                valid.lastOrNull()
    ...
    private fun onChanged() {
        ...
        fileState =
            if (workspaceFilePath != null) {
                "Modified - local saved - sync pending"
            } else {
                "Local saved"
            }
    ...
    fun forceSync() {
        ...
        if (path == null) {
            toast(
                "No JSON linked. Use Save JSON first.",
                ToastType.Warning
            )
            return
        }
    ...
    fun openWorkspaceFileDialog() {
        if (dirty) {
            val ok = javax.swing.JOptionPane.showConfirmDialog(
                null,
                "You have local changes. Open another JSON?",
                "Confirm",
        ...
        if (f.name != "axon-workspace.json") {
            val ok = javax.swing.JOptionPane.showConfirmDialog(
                null,
                "For auto-linking, file must be named axon-workspace.json. Continue?",
                "Confirm",
                javax.swing.JOptionPane.YES_NO_OPTION
            )
    ...
    fun saveWorkspaceFileDialog() {
        ...
        meta.lastSyncedRemoteModified =
            workspaceFileLastModified

        meta.lastSyncedLocalUpdatedAt =
            meta.localUpdatedAt
    ...
    fun importWorkspaceDialog() {
        try {
            ...
            val existingIds = workspace.projects
                .map { it.id }
                .toMutableSet()

            val existingNames = workspace.projects
                .map { it.name.lowercase() }
                .toMutableSet()

            val importedProjects = imported.projects.map { p ->
                var newId = p.id
                ...
                if (name.lowercase() in existingNames) {
                    ...
                    while (
                        "$name $i".lowercase() in existingNames
                    ) {
                        i++
                    }
            ...
            val selectedId =
                importedProjects.lastOrNull()?.id
                    ?: return

            val newProjects =
                workspace.projects.toMutableList().apply {
                    addAll(importedProjects)
                }
            ...
            println(
                "[Import] Added ${importedProjects.size} projects"
            )

            toast(
                "Imported ${importedProjects.size} project" +
                        if (importedProjects.size == 1) "" else "s",
                ToastType.Info
            )
    ...
    private fun tryParseAny(text: String): Workspace? {
        runCatching {
            val w = Persistence.json.decodeFromString(
                Workspace.serializer(),
                text
            )
        ...
        root["projects"]?.let {
            return runCatching {
                Persistence.json.decodeFromString(
                    Workspace.serializer(),
                    text
                )
            }.getOrNull()
        ...
        fun str(key: String): String? =
            (root[key] as? JsonPrimitive)?.content
        ...
        val name =
            nameKeys.firstNotNullOfOrNull {
                str(it)
            }

        val id =
            idKeys.firstNotNullOfOrNull {
                str(it)
            } ?: newId()

        val treeEl =
            treeKeys.firstNotNullOfOrNull {
                root[it]
            } ?: return null

        val tree = runCatching {
            Persistence.json.decodeFromJsonElement(
                Node.serializer(),
                treeEl
            )
        }.getOrNull() ?: return null
    ...
    private fun loadRemoteFile(
        ...
        val parsed =
            runCatching {
                ...
                ?: run {
                    fileState = "JSON load failed"
                    return
                }

        if (
            !force &&
            sharedChangesExist(f.lastModified())
        ) {
            fileState = "Conflict - local changes kept"
            return
        }
        ...
        val validId =
            if (parsed.projects.any {
                    it.id == parsed.activeId
                }
            ) {
                parsed.activeId
            } else {
                parsed.projects.firstOrNull()?.id
            }
    ...
    private fun restoreLinkedWorkspace() {
        ...
        // 2) Auto-find axon-workspace.json in app root
        val appRoot = Persistence.appRootDir()
        ...
        if (appRoot != null) {
            ...
            if (candidate.exists()) {
                ...
                println(
                    "[AutoLink] Found: ${candidate.absolutePath}"
                )
    ...
    private fun sharedChangesExist(
        fileModified: Long
    ): Boolean =
    ...
    private fun syncToJson(force: Boolean) {
        ...
        if (
            !force &&
            f.exists() &&
            workspaceFileLastModified != 0L &&
            f.lastModified() != workspaceFileLastModified
        ) {
            fileState = "Conflict - remote changed"
            return
        }
        ...
        runCatching {
            ...
            fileState =
                "Local saved - JSON sync unavailable"
            return
        }
    ...
    fun copySelected() {
        ...
        val ids =
            selectedNodeIds.ifEmpty {
                setOfNotNull(selectedNodeId)
            }
        ...
        toast(
            "Copied ${clipboard.size} item" +
                    if (clipboard.size == 1) "" else "s",
            ToastType.Info
        )
    }
    ...
    fun pasteIntoSelection() {
        ...
        if (lastChild != null) {
            selectOnly(lastChild.id)
        }

        toast(
            "Pasted ${clipboard.size} item" +
                    if (clipboard.size == 1) "" else "s",
            ToastType.Success
        )
    }

    private fun parentForPaste(
        p: Project
    ): Node? {
    ...
    private fun deepCloneNode(
        ...
        return Node(
            ...
            open = n.open,
            children = n.children
                .map { deepCloneNode(it) }
                .toMutableList()
        )
    }

    private fun regenerateIds(
        n: Node
    ): Node {
        val newChildren = n.children
            .map { c ->
                regenerateIds(
                    c.copy(id = newId())
                )
